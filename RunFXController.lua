...
local function startElectricSoundSequence()
	...
	electricSoundCoroutine = task.spawn(function()
		...
		while true do
			...
			-- No need to check IsLoaded here, as we are preloading them
...
local function initializeSounds()
	...
	for _, id in ipairs(ELECTRIC_SOUND_IDS) do
		...
		sound.SoundId, sound.Volume, sound.Parent = id, 0.6, cam -- Trail lightning is now quieter
		table.insert(electricSoundPool, sound)
	end
	-- FIX: Preload sounds to prevent delay on first run
	for _, sound in ipairs(electricSoundPool) do
		sound:Play()
		sound:Stop()
	end
...
local function startSprint()
	...
	-- NEW: Play the louder electric burst sound
	local burst = ElectricBurstSample:Clone()
	burst.Volume = 1.0 -- Make the burst intelligible over the trail sound
	burst.Parent = cam
	burst:Play()
